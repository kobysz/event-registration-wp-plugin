...
Roadmapa 6 planów, każdy w cyklu spec → plan → wykonanie subagentami (superpowers). Scalone do master: **Plan 1 (domena), Plan 2 (CPT + REST + React admin), Plan 3A (transakcyjny backend rezerwacji), Plan 3B (formularz publiczny)**. Kolejny: Plan 4 (kolejka mailowa — double opt-in, retry, wygasanie; token i endpoint potwierdzenia już działają z 3B). Specyfikacje i plany w `docs/superpowers/`.
